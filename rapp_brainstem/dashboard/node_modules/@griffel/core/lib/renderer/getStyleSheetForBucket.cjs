...
function isSameInsertionKey(element, bucketName, metadata) {
    ...
    const elementKey = element.getAttribute(_constants.DATA_BUCKET_ATTR) + ((_b = element.media) !== null && _b !== void 0 ? _b : '');
 ...
 */ function findInsertionPoint(targetDocument, insertionPoint, targetBucket, renderer, metadata = {}) {
    ...
    if (targetBucket === 'm') {
        const mediaElements = targetDocument.head.querySelectorAll(`[${_constants.DATA_BUCKET_ATTR}="${targetBucket}"]`);
