...
export class OutlineAPI {
    ...
    _onKeyboardNavigationStateChanged = () => {
    ...
    _onScroll = (e) => {
        if (!this._outlinedElement ||
            !OutlineAPI._isParentChild(e.target, this._outlinedElement)) {
            return;
        }
        this._curPos = undefined;
    ...
    _setOutlinePosition() {
        ...
        const win = this._outlinedElement.ownerDocument &&
        ...
        const scrollingElement = container &&
            container.ownerDocument &&
            container.ownerDocument.scrollingElement;
        if (!scrollingElement) {
            return;
        }
        for (let parent = this._outlinedElement.parentElement; parent && parent.nodeType === Node.ELEMENT_NODE; parent = parent.parentElement) {
            ...
            if ((!hasAbsolutePositionedParent && !hasFixedPositionedParent) ||
        ...
        if (width > ow * 2 && height > ow * 2) {
            ...
            const sx = this._fullScreenElement || hasFixedPositionedParent
                ? 0
                : win.pageXOffset;
            const sy = this._fullScreenElement || hasFixedPositionedParent
                ? 0
                : win.pageYOffset;
    ...
    _removeDOM(contextElement) {
        const win = (contextElement.ownerDocument &&
            contextElement.ownerDocument.defaultView);
