...
class GroupperDummyManager extends _DummyInput.DummyInputManager {
    constructor(element, groupper, tabster, sys){
        super(tabster, element, _DummyInput.DummyInputManagerPriorities.Groupper, sys, true);
        this._setHandlers((dummyInput, isBackward, relatedTarget)=>{
            const container = element.get();
            const input = dummyInput.input;
            if (container && input) {
                const ctx = _Root.RootAPI.getTabsterContext(tabster, input);
                if (ctx) {
                    let next;
                    next = groupper.findNextTabbable(relatedTarget || undefined, undefined, isBackward, true)?.element;
                    if (!next) {
                        next = _FocusedElement.FocusedElementState.findNextTabbable(tabster, ctx, undefined, dummyInput.isOutside ? input : (0, _Utils.getAdjacentElement)(container, !isBackward), undefined, isBackward, true)?.element;
                    }
                    if (next) {
...
class Groupper extends _Utils.TabsterPart {
    ...
    findNextTabbable(currentElement, referenceElement, isBackward, ignoreAccessibility) {
        ...
        const currentIsDummy = (0, _DummyInput.getDummyInputContainer)(currentElement) === groupperElement;
        if (!this._shouldTabInside && currentElement && _DOMAPI.dom.nodeContains(groupperElement, currentElement) && !currentIsDummy) {
            return {
                element: undefined,
                outOfDOMOrder: true
            };
        ...
        if (!currentElement || !_DOMAPI.dom.nodeContains(groupperElement, currentElement) || currentIsDummy) {
            return {
        ...
        if (this._shouldTabInside && groupperFirstFocusable) {
            ...
            if (!next && this._props.tabbability === _Consts.GroupperTabbabilities.LimitedTrapFocus) {
    ...
    isActive(noIfFirstIsFocused) {
        ...
        for(let e = _DOMAPI.dom.getParentElement(element); e; e = _DOMAPI.dom.getParentElement(e)){
            const g = (0, _Instance.getTabsterOnElement)(this._tabster, e)?.groupper;
            if (g) {
    ...
    setFirst(element) {
        if (element) {
            this._first = new _Utils.WeakHTMLElement(element);
        } else {
            delete this._first;
        }
    }
    acceptElement(element, state) {
        ...
        const parentElement = _DOMAPI.dom.getParentElement(this.getElement());
        const parentCtx = parentElement && _Root.RootAPI.getTabsterContext(this._tabster, parentElement);
        const parentCtxGroupper = parentCtx?.groupper;
        const parentGroupper = parentCtx?.groupperBeforeMover ? parentCtxGroupper : undefined;
        ...
        if (parentGroupper) {
            ...
            if (!getIsActive(parentGroupper) && parentGroupperElement && state.container !== parentGroupperElement && _DOMAPI.dom.nodeContains(state.container, parentGroupperElement)) {
        ...
        if (groupperElement) {
            if (isActive !== true) {
                if (groupperElement === element && parentCtxGroupper) {
                    ...
                    if (parentGroupperElement && !getIsActive(parentCtxGroupper) && _DOMAPI.dom.nodeContains(state.container, parentGroupperElement) && parentGroupperElement !== state.container) {
                        state.skippedFocusable = true;
                        return NodeFilter.FILTER_REJECT;
                    }
                }
                if (groupperElement !== element && _DOMAPI.dom.nodeContains(groupperElement, element)) {
                    state.skippedFocusable = true;
                    return NodeFilter.FILTER_REJECT;
                }
...
class GroupperAPI {
    ...
    _init = ()=>{
        ...
        win.addEventListener(_Events.GroupperMoveFocusEventName, this._onMoveFocus);
    };
    dispose() {
        const win = this._win();
        this._tabster.focusedElement.cancelAsyncFocus(_Consts.AsyncFocusSources.EscapeGroupper);
        this._current = {};
        ...
        win.removeEventListener(_Events.GroupperMoveFocusEventName, this._onMoveFocus);
    ...
    createGroupper(element, props, sys) {
        ...
        if (focusedElement && _DOMAPI.dom.nodeContains(element, focusedElement) && !this._updateTimer) {
    ...
    _updateCurrent(element) {
        ...
        for(let el = tabster.getParent(element); el; el = tabster.getParent(el)){
            const groupper = (0, _Instance.getTabsterOnElement)(tabster, el)?.groupper;
            if (groupper) {
    ...
    _onKeyDown = (event)=>{
        if (event.key !== _Keys.Keys.Enter && event.key !== _Keys.Keys.Escape) {
            return;
        }
    ...
    _onMoveFocus = (e)=>{
        ...
        if (element && action !== undefined && !e.defaultPrevented) {
            if (action === _Consts.GroupperMoveFocusActions.Enter) {
    ...
    _enterGroupper(element, relatedEvent) {
        ...
        if (groupper && groupperElement && (element === groupperElement || groupper.getProps().delegated && element === groupper.getFirst(false))) {
            ...
            if (next && (!relatedEvent || relatedEvent && groupperElement.dispatchEvent(new _Events.TabsterMoveFocusEvent({
                by: "groupper",
    ...
    _escapeGroupper(element, relatedEvent, fromModalizer) {
        ...
        const groupperElement = groupper?.getElement();
        if (groupper && groupperElement && _DOMAPI.dom.nodeContains(groupperElement, element)) {
            let next;
            if (element !== groupperElement || fromModalizer) {
                next = groupper.getFirst(true);
            } else {
                const parentElement = _DOMAPI.dom.getParentElement(groupperElement);
                const parentCtx = parentElement ? _Root.RootAPI.getTabsterContext(tabster, parentElement) : undefined;
    ...
    moveFocus(element, action) {
        return action === _Consts.GroupperMoveFocusActions.Enter ? this._enterGroupper(element) : this._escapeGroupper(element);
    }
    handleKeyPress(element, event, fromModalizer) {
        const tabster = this._tabster;
        const ctx = _Root.RootAPI.getTabsterContext(tabster, element);
        if (ctx && (ctx?.groupper || ctx?.modalizerInGroupper)) {
            tabster.focusedElement.cancelAsyncFocus(_Consts.AsyncFocusSources.EscapeGroupper);
            if (ctx.ignoreKeydown(event)) {
                return;
            }
            if (event.key === _Keys.Keys.Enter) {
                this._enterGroupper(element, event);
            } else if (event.key === _Keys.Keys.Escape) {
                ...
                const focusedElement = tabster.focusedElement.getFocusedElement();
                tabster.focusedElement.requestAsyncFocus(_Consts.AsyncFocusSources.EscapeGroupper, ()=>{
