...
class DeloserItem extends DeloserItemBase {
    ...
    async focusAvailable() {
        ...
        if (available && deloserElement) {
            if (!deloserElement.dispatchEvent(new _Events.TabsterMoveFocusEvent({
                by: "deloser",
...
function _setInformativeStyle(weakElement, remove, isActive, snapshotIndex) {
...
function buildSelector(element) {
    if (!(0, _Utils.documentContains)(element.ownerDocument, element)) {
        return undefined;
    }
    const selector = [
        buildElementSelector(element)
    ];
    let node = _DOMAPI.dom.getParentNode(element);
    while(node && node.nodeType !== Node.DOCUMENT_FRAGMENT_NODE){
        ...
        node = _DOMAPI.dom.getParentNode(node);
    }
    return selector.join(" ");
}
class Deloser extends _Utils.TabsterPart {
    uid;
    strategy;
    _isActive = false;
    _history = [
    ...
    constructor(tabster, element, onDispose, props){
        ...
        this.uid = (0, _Utils.getElementUId)(tabster.getWindow, element);
        this.strategy = props.strategy || _Consts.DeloserStrategies.Auto;
    ...
    unshift(element) {
        ...
        cur.unshift(new _Utils.WeakHTMLElement(element, buildSelector(element)));
    ...
    findAvailable() {
        ...
        if (restoreFocusOrder === _Consts.RestoreFocusOrders.RootDefault) {
            available = this._tabster.focusable.findDefault({
                container: rootElement
            });
        }
        if (!available && restoreFocusOrder === _Consts.RestoreFocusOrders.RootFirst) {
        ...
        if (availableInHistory && restoreFocusOrder === _Consts.RestoreFocusOrders.History) {
            return availableInHistory;
        }
        const availableDefault = this._tabster.focusable.findDefault({
            container: element
        });
        if (availableDefault && restoreFocusOrder === _Consts.RestoreFocusOrders.DeloserDefault) {
            return availableDefault;
        }
        const availableFirst = this._findFirst(element);
        if (availableFirst && restoreFocusOrder === _Consts.RestoreFocusOrders.DeloserFirst) {
    ...
    clearHistory = (preserveExisting)=>{
        ...
        this._history[this._snapshotIndex] = this._history[this._snapshotIndex].filter((we)=>{
            const e = we.get();
            return e && preserveExisting ? _DOMAPI.dom.nodeContains(element, e) : false;
        });
    };
    customFocusLostHandler(element) {
        return element.dispatchEvent(new _Events.DeloserFocusLostEvent(this.getActions()));
    }
    _findInHistory() {
        ...
        for(let i = 0; i < cur.length; i++){
            ...
            const e = we.get();
            const element = this._element.get();
            if (e && element && _DOMAPI.dom.nodeContains(element, e)) {
                ...
                if (selector && element) {
                    let els;
                    try {
                        els = _DOMAPI.dom.querySelectorAll(element.ownerDocument, selector);
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
function validateDeloserProps(props) {
...
class DeloserAPI {
    ...
    constructor(tabster, props){
        ...
        tabster.queueInit(()=>{
            ...
            doc.addEventListener(_Events.DeloserRestoreFocusEventName, this._onRestoreFocus);
    ...
    dispose() {
        ...
        win.document.removeEventListener(_Events.DeloserRestoreFocusEventName, this._onRestoreFocus);
    ...
    createDeloser(element, props) {
        ...
        if (_DOMAPI.dom.nodeContains(element, this._tabster.focusedElement.getFocusedElement() ?? null)) {
            this._activate(deloser);
        }
        return deloser;
    }
    getActions(element) {
        for(let e = element; e; e = _DOMAPI.dom.getParentElement(e)){
    ...
    _scheduleRestoreFocus(force) {
        ...
        const restoreFocus = async ()=>{
            ...
            if (!force && (this._isRestoringFocus || !this._inDeloser || lastFocused && !(0, _Utils.isDisplayNone)(lastFocused))) {
                return;
            }
            const curDeloser = this._curDeloser;
            let isManual = false;
            if (curDeloser) {
                ...
                if (curDeloser.strategy === _Consts.DeloserStrategies.Manual) {
                    isManual = true;
                } else {
                    const curDeloserElement = curDeloser.getElement();
                    const el = curDeloser.findAvailable();
                    if (el && (!curDeloserElement?.dispatchEvent(new _Events.TabsterMoveFocusEvent({
                        by: "deloser",
    ...
    static getDeloser(tabster, element) {
        let root;
        for(let e = element; e; e = _DOMAPI.dom.getParentElement(e)){
            const tabsterOnElement = (0, _Instance.getTabsterOnElement)(tabster, e);
            if (tabsterOnElement) {
                if (!root) {
...
} //# sourceMappingURL=Deloser.js.map
