...
const _AttributeHelpers = require("./AttributeHelpers.cjs");
...
class RootDummyManager extends _DummyInput.DummyInputManager {
    _tabster;
    _setFocused;
    constructor(tabster, element, setFocused, sys){
        super(tabster, element, _DummyInput.DummyInputManagerPriorities.Root, sys, undefined, true);
...
class Root extends _Utils.TabsterPart {
    uid;
    _dummyManager;
    _sys;
    _isFocused = false;
    ...
    constructor(tabster, element, onDispose, props, sys){
        ...
        this.uid = (0, _Utils.getElementUId)(win, element);
        this._sys = sys;
        ...
        doc.addEventListener(_keyborg.KEYBORG_FOCUSIN, this._onFocusIn);
        doc.addEventListener(_keyborg.KEYBORG_FOCUSOUT, this._onFocusOut);
        this._add();
    }
    addDummyInputs() {
    ...
    dispose() {
        ...
        doc.removeEventListener(_keyborg.KEYBORG_FOCUSIN, this._onFocusIn);
        doc.removeEventListener(_keyborg.KEYBORG_FOCUSOUT, this._onFocusOut);
    ...
    _setFocused = (hasFocused)=>{
        ...
        if (element) {
            if (hasFocused) {
                ...
                element.dispatchEvent(new _Events.RootFocusEvent({
                    element
                }));
            } else {
                this._setFocusedTimer = this._tabster.getWindow().setTimeout(()=>{
                    ...
                    element.dispatchEvent(new _Events.RootBlurEvent({
                        element
                    }));
                }, 0);
            }
...
class RootAPI {
    ...
    _autoRootCreate = ()=>{
        ...
        if (body) {
            ...
            if (props) {
                (0, _AttributeHelpers.setTabsterAttribute)(body, {
                    root: props
                }, true);
                (0, _Instance.updateTabsterByAttribute)(this._tabster, body);
                return (0, _Instance.getTabsterOnElement)(this._tabster, body)?.root;
            }
     ...
     */ static getTabsterContext(tabster, element, options = {}) {
        ...
        while(curElement && (!root || checkRtl)){
            const tabsterOnElement = (0, _Instance.getTabsterOnElement)(tabster, curElement);
    ...
    static getRoot(tabster, element) {
        ...
        for(let el = element; el; el = getParent(el)){
            const root = (0, _Instance.getTabsterOnElement)(tabster, el)?.root;
            if (root) {
                return root;
            }
        }
