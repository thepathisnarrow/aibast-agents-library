...
_export(exports, {
    ...
    get CrossOriginElement () {
        return CrossOriginElement;
    },
    get CrossOriginFocusedElementState () {
        return CrossOriginFocusedElementState;
    },
    get CrossOriginObservedElementState () {
...
const _Subscribable = require("./State/Subscribable.cjs");
...
class CrossOriginDeloserItem extends _Deloser.DeloserItemBase {
    _deloser;
    _transactions;
...
class CrossOriginDeloserHistoryByRoot extends _Deloser.DeloserHistoryByRootBase {
    _transactions;
...
class CrossOriginTransaction {
    ...
    constructor(tabster, getOwner, knownTargets, value, timeout, sentTo, targetId, sendUp){
        ...
        this.owner = getOwner;
        this.ownerId = (0, _Utils.getWindowUId)(getOwner());
        this.id = (0, _Utils.getUId)(getOwner());
    ...
    static async _makeFocusedResponse(tabster, timestamp, beginData, transactions, isSelfResponse) {
        ...
        if (beginData && beginData.ownerUId && element) {
            ...
            if (!isSelfResponse && beginData.rootUId && beginData.deloserUId) {
                ...
                if (deloserAPI) {
                    const history = _Deloser.DeloserAPI.getHistory(deloserAPI);
                    const deloser = {
    ...
    static async _makeDeadWindowResponse(tabster, beginData, transactions, forwardResult) {
        ...
        return forwardResult.then(()=>{
            if (deadUId === transactions.ctx.focusOwner) {
                const deloserAPI = tabster.deloser;
                if (deloserAPI) {
                    _Deloser.DeloserAPI.forceRestoreFocus(deloserAPI);
                }
            }
            return true;
        });
...
class GetElementTransaction extends CrossOriginTransaction {
    ...
    static findElement(tabster, getOwner, data) {
        let element;
        if (data && (!data.ownerId || data.ownerId === (0, _Utils.getWindowUId)(getOwner()))) {
            if (data.id) {
                element = _DOMAPI.dom.getElementById(getOwner().document, data.id);
                ...
                const ref = (0, _Utils.getInstanceContext)(getOwner).elementByUId[data.uid];
                element = ref && ref.get();
    ...
    static getElementData(tabster, element, getOwner, context, ownerUId) {
        const deloser = _Deloser.DeloserAPI.getDeloser(tabster, element);
        const ctx = _Root.RootAPI.getTabsterContext(tabster, element);
        const tabsterOnElement = (0, _Instance.getTabsterOnElement)(tabster, element);
        const observed = tabsterOnElement && tabsterOnElement.observed;
        return {
            uid: (0, _Utils.getElementUId)(getOwner, element),
            ownerUId,
...
class RestoreFocusInDeloserTransaction extends CrossOriginTransaction {
    ...
    static async makeResponse(tabster, data, getOwner, ownerId, transactions, forwardResult) {
        ...
        if (begin && deloser && deloserAPI) {
            const history = _Deloser.DeloserAPI.getHistory(deloserAPI);
            const result = begin.reset ? await history.resetFocus(deloser) : await history.focusAvailable(deloser);
            return result ?? false;
        }
        return !!forwardRet;
    }
...
class CrossOriginTransactions {
    ...
    constructor(tabster, getOwner, context){
        ...
        this._ownerUId = (0, _Utils.getWindowUId)(getOwner());
        this.ctx = context;
    }
    setup(sendUp) {
        if (this.isSetUp) {
...
class CrossOriginFocusedElementState extends _Subscribable.Subscribable {
    _transactions;
...
class CrossOriginObservedElementState extends _Subscribable.Subscribable {
    _tabster;
    _transactions;
    ...
    async requestFocus(observedName, timeout) {
        ...
        return this.waitElement(observedName, timeout, _Consts.ObservedElementAccessibilities.Focusable).then((element)=>this._lastRequestFocusId === requestId && element ? this._tabster.crossOrigin.focusedElement.focus(element, true) : false);
    }
...
class CrossOriginAPI {
    ...
    _onKeyboardNavigationStateChanged = (value)=>{
        if (!this._ctx.ignoreKeyboardNavigationStateUpdate) {
            this._transactions.beginTransaction(StateTransaction, {
                ...
                ownerUId: (0, _Utils.getWindowUId)(this._win()),
    ...
    _onFocus = (element)=>{
        const win = this._win();
        const ownerUId = (0, _Utils.getWindowUId)(win);
        if (this._blurTimer) {
    ...
    _onObserved = (element, details)=>{
        const d = GetElementTransaction.getElementData(this._tabster, element, this._win, this._ctx, (0, _Utils.getWindowUId)(this._win()));
...
function getDeloserUID(getWindow, context, deloser) {
    ...
    if (deloserElement) {
        const uid = (0, _Utils.getElementUId)(getWindow, deloserElement);
